...
Headers (search-service). No live Meilisearch/document-service needed:
the gateway_auth middleware runs BEFORE routing, so a rejected request
never reaches a router, and a request that clears the middleware can be
aimed at a nonexistent path to prove it reached FastAPI's own routing
(a 404) rather than being caught by the guard.
"""
...
def test_request_with_wrong_secret_is_rejected():
    resp = client.get("/search", headers={"X-Internal-Secret": "definitely-not-the-secret"})
    assert resp.status_code == 401


def test_spoofed_trust_headers_without_secret_cannot_escalate():
    resp = client.get("/search", headers={"X-Allowed-Projects": "ALL"})
    assert resp.status_code == 401
